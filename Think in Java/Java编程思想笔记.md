...
​	十六进制数适用于所有整数数据类型，以前缀**0x（或0X），后面跟随0-9或小写（或大写）a-f**来表示。**如果试图将一个变量初始化成超出自身范围的值（无论这个值的数值形式如何），编译器都会向我们报告一条错误信息**。注意在前面的代码中，已经给出了char、byte和short所能表示的最大的十六进制值。如果超出范围，编译器会将值自动转成int型，并告诉我们需要对这次赋值进行**窄化转型**（**将能容纳更多信息的数据类型转换为无法容纳那么多信息的类型**），这样我们就可清楚地知道自己的操作是否越界了。
